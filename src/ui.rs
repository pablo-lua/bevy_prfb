...
use bevy::{
    ecs::{
        ...
        event::EventWriter,
        query::Changed,
        system::{Commands, Query, Resource, SystemId},
    },
    log::{error, warn},
    prelude::{Entity, Event, Plugin, Update},
    ui::Interaction,
};
...
use crate::{
    ...
    prefab::{Format, Prefab, PrefabCommands, PrefabCommandsExt, PrefabData, PrefabPlugin},
};

use super::components::ui::{
    ButtonBundlePrefab, ImageBundlePrefab, NodeBundlePrefab, TextBundlePrefab,
};
...
where
    C: CustomWidget + for<'a> Deserialize<'a>,
{
    fn load_from_bytes(
        bytes: Vec<u8>,
    ) -> Result<Prefab<UiData<C::CustomData>>, Box<dyn std::error::Error>> {
    ...
    match widget {
        ...
        Ui::Text {
            ...
            prefab
                ...
                .set_data((None, Some(text_data), None, None, None, None, custom_data));
        }
        Ui::Image {
            image_data,
            custom_data,
            ...
            prefab
                .get_entity_mut(index)
                .expect("Unreachable: `Prefab` entity should always be set when walking ui tree")
                .set_data((None, None, Some(image_data), None, None, None, custom_data));
        }
        Ui::Button {
            button,
            label,
            ...
            prefab
                .get_entity_mut(index)
                .expect("Unreachable: `Prefab` entity should always be set when walking ui tree")
                .set_data((None, None, None, Some(button), callback, None, custom_data));

            if let Some(text) = label {
                prefab.add(
                    Some(index),
                    Some((None, Some(text), None, None, None, None, None)),
                );
            }
        }
        Ui::Container {
            ...
            prefab
                .get_entity_mut(index)
                .expect("Unreachable: `Prefab` entity should always be set when walking ui tree")
                .set_data((Some(node), None, None, None, None, other_data, custom_data));
...
pub struct UiPrefabPlugin<C = NoCustomWidget>(PhantomData<C>)
where
    C: CustomWidget;
impl<C: CustomWidget> Plugin for UiPrefabPlugin<C> {
    fn build(&self, app: &mut bevy::prelude::App) {
        app.add_plugins(PrefabPlugin::<UiData<C::CustomData>>::new())
...
pub struct UiCreator<S, C = NoCustomWidget>(pub S, pub PhantomData<C>)
where
    C: CustomWidget,
    S: AsRef<Path>;
...
where
    C: CustomWidget + for<'a> Deserialize<'a>,
{
    fn load_ui<S: AsRef<Path>>(
        &'c mut self,
        creator: UiCreator<S, C>,
    ) -> PrefabCommands<'c, 'w, 's, UiData<C::CustomData>>;
}

impl<'c, 'w, 's, C: CustomWidget + for<'a> Deserialize<'a>> CreateUiExt<'c, 'w, 's, C>
    for Commands<'w, 's>
{
    fn load_ui<S: AsRef<Path>>(
        &'c mut self,
...
impl PrefabData for CallbackPrefab {
    fn insert_into_entity(self, entidade: &mut bevy::prelude::EntityWorldMut) {
        let callback = match self {
            Self::LoadedSystem(s) => Some(CallBack::RegisteredSystem(s)),
            Self::Event { name } => Some(CallBack::EventSender(PressedButtonEvent::new(
                name,
                entidade.id(),
            ))),
            Self::System(_) => {
...
pub fn handle_prefab_events(
    ...
    mut events: EventWriter<PressedButtonEvent>,
    buttons_callback: Query<(&CallBack, &Interaction), Changed<Interaction>>,
) {
    for (callback, interaction) in buttons_callback.iter() {
        match callback {
            CallBack::EventSender(e) => events.send(PressedButtonEvent {
                button_name: e.button_name.clone(),
                entity: e.entity,
